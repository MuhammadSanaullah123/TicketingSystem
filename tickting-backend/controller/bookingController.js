...
exports.assignSeats = catchAsync(async (req, res, next) => {
    ...
    res.json({ success: true, message: "Seats Assigned Successfully", bookingDetails: result })
})

exports.getSeats = catchAsync(async (req, res, next) => {
    let { tripId, seats, phone, noOfSeats, email, price, passengerDetails } = req.body
    ...
    if (noOfSeats == 0 && !noOfSeats) {
        return next(
            new AppError("You Must Provide Atleast One Seat.")
        )
    }
    ...
    busDetails.availableSeats = busDetails.totalSeats - busDetails.occupiedSeats.length - noOfSeats;
...
exports.bookSeatsWithoutLogin = catchAsync(async (req, res, next) => {
    let { busId, phone, noOfSeats, email, price, passengerDetails, additionalDetails } = req.body;
  ...
  console.log("receivedData?.seats",checkVariable);
    ...
    if (busDetails.seatSelection) {
        if (seats.length !== parseInt(noOfSeats)) return next(new AppError('Enter valid number of seats', 400))
        for (let i = 0; i < seats.length; ++i) {
            if (seats[i] < 0 || seats[i] > busDetails.totalSeats) return next(new AppError('Enter a valid seat number', 400));
            let isSeatBooked = busDetails.occupiedSeats.includes(seats[i]);
            if (busDetails.disabledSeats.includes(seats[i])) return next(new AppError('Selected seat is disabled', 400));
            console.log(isSeatBooked);
            if (isSeatBooked) {
                return next(
                    new AppError("Seat Is Already Occupied")
...
exports.getBooking = catchAsync(async (req, res, next) => {
    const booking = await Booking.findById(req.params.id).populate('busId').populate('operatorId')
    if (!booking) {
        return next(
            new AppError('Invalid Booking Id Provided', 404)
        )
    }
    res.json(booking)
})
...
exports.getParticularBooking = catchAsync(async (req, res, next) => {
    ...
    if (!booking) {
        return next(
            new AppError('Invalid Booking id Provided', 403)
        )
    }
...
exports.cancelBooking = catchAsync(async (req, res, next) => {
    ...
    if (booking.seats.length > 0) {
        for (let i = 0; i < booking.seats.length; i++) {
            bus.occupiedSeats = bus.occupiedSeats.filter(b => b != booking.seats[i])
        }
    }
    await bus.save()
    await booking.save()
